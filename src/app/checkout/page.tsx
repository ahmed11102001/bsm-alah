...
function CheckoutContent() {
  ...
  // ── نوع الشراء: اشتراك باقة (plan) أو حزمة إضافية لمرة واحدة (packageId) ──
  // packageId بييجي من زراير زي "MCP Addon" في صفحة الاستهلاك، وهو مختلف
  // تمامًا عن الاشتراكات (مفيش billing cycle، مفيش كوبون خاص بيه، السعر ثابت).
  // من غير الفحص ده، أي packageId كان بيترجم بالغلط لباقة Pro الشهرية الافتراضية.
  const packageId = params.get("packageId");
  const addonPackage = packageId
  ...
  const originalPrice = isAddonPurchase ? (addonPackage?.priceEGP ?? 0) : monthlyPrice * cycle.months;
  const discount = !isAddonPurchase && couponState
  ...
  const whatsappMessage = useMemo(() => [
    ...
    isAddonPurchase ? `الحزمة: ${productName}` : `الباقة: ${productName}`,
    ...
    !isAddonPurchase && couponState ? `الكوبون: ${couponState.code}` : "",
    "سأرسل Screenshot لإيصال الدفع في هذه المحادثة.",
  ].filter(Boolean).join("\n"), [productName, originalPrice, discount, appliedCredit, finalPrice, paymentMethod, couponState, isAddonPurchase]);

  async function applyCoupon() {
    if (isAddonPurchase) return; // الكوبونات مخصوصة بالاشتراكات فقط
  ...
  // ── packageId موجود بس مش موجود في الباقات المعروفة — أوقف هنا بدل ما نكمل
  // بسعر 0 EGP أو باسم "حزمة غير معروفة" ────────────────────────────────────
  if (isAddonPurchase && !addonPackage) {
    return <main dir="rtl" className="flex min-h-screen flex-col items-center justify-center gap-3 bg-[#f7faf8] px-4 text-center">
      <p className="text-lg font-black text-gray-800">الحزمة المطلوبة غير موجودة</p>
      <p className="text-sm text-gray-500">تأكد من الرابط أو تواصل مع الدعم الفني.</p>
      <button onClick={() => router.back()} className="mt-2 rounded-xl bg-gray-900 px-4 py-2 text-sm font-bold text-white">رجوع</button>
    </main>;
  }
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f7faf8] text-gray-900">
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:py-12">
      <div className="mb-8 text-center"><h1 className="text-2xl font-black sm:text-3xl">{isAddonPurchase ? "إتمام الشراء" : "إتمام الاشتراك"}</h1><p className="mt-2 text-sm text-gray-500">أكمل خطوات الدفع لتفعيل {isAddonPurchase ? "حزمتك" : "باقتك"}</p></div>
      <div className="grid items-start gap-5 lg:grid-cols-5">
        <section className="space-y-5 lg:col-span-2">
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <p className="mb-4 text-xs font-bold text-gray-400">{isAddonPurchase ? "ملخص الحزمة" : "ملخص الباقة"}</p>
            <div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-black">{productName}</h2><p className="mt-1 text-xs text-gray-500">{isAddonPurchase ? addonPackage?.description : plan.tagline}</p></div><CreditCard className="h-7 w-7 text-[#25D366]" /></div>
            ...
            {!isAddonPurchase && showFeatures && <ul className="mt-3 space-y-2">{plan.features.map((feature, i) => <li key={i} className="flex gap-2 text-xs text-gray-600"><Check className="h-3.5 w-3.5 shrink-0 text-[#25D366]" />{feature}</li>)}</ul>}
            {!isAddonPurchase && <button onClick={() => setShowFeatures(v => !v)} className="mt-4 flex w-full items-center justify-between text-xs text-gray-500"><span>أهم مميزات الباقة</span>{showFeatures ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>}
          </div>
          {referralCredit > 0 && (
          ...
          {!isAddonPurchase && <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><div className="mb-3 flex items-center gap-2 text-xs font-bold text-gray-500"><Tag className="h-4 w-4" /> لديك كوبون خصم؟</div><div className="flex gap-2"><input value={coupon} onChange={e => { setCoupon(e.target.value.toUpperCase()); setCouponError(""); setCouponState(null); }} placeholder="WAN10" className="min-w-0 flex-1 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-mono outline-none focus:border-[#25D366]" /><button onClick={applyCoupon} disabled={!coupon.trim() || couponLoading} className="rounded-xl bg-gray-900 px-4 text-sm font-bold text-white disabled:opacity-40">{couponLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "تطبيق"}</button></div>{couponError && <p className="mt-2 text-xs text-red-500">{couponError}</p>}{couponState && <div className="mt-2 flex items-center justify-between text-xs text-[#1a9e50]"><span>تم تطبيق الكوبون {couponState.code}</span><button onClick={() => setCouponState(null)}><X className="h-3.5 w-3.5" /></button></div>}</div>}
        </section>
