            ...
            <DialogContent className="max-w-lg bg-card border-border">
                <DialogHeader>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <DialogTitle className="font-mono text-base text-foreground">{template.name}</DialogTitle>
                ...
                <div className="space-y-4 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                        {[
                            ...
                            <div key={r.label} className="bg-muted/70 rounded-xl p-3">
                                <p className="text-muted-foreground mb-0.5">{r.label}</p>
                                <p className="font-mono font-medium text-foreground break-all text-[11px]">{r.value}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    {template.body && (
                        <div>
                            <p className="text-xs font-semibold text-muted-foreground mb-1.5">{t.body.replace(" *", "")}</p>
                            <div className="bg-muted/70 rounded-xl p-3 border border-border">
                                <p className="text-sm text-foreground/80 whitespace-pre-wrap leading-relaxed">{template.body}</p>
                            </div>
                        </div>
                    )}
                    ...
                    {template.buttons && template.buttons.length > 0 && (
                        <div className="space-y-1.5">
                            ...
                            {template.buttons.map((btn, i) => (
                                <div key={i} className="flex items-center gap-2 bg-muted/70 rounded-lg px-3 py-2 text-xs">
                                    ...
                                    {btn.value && <span className="text-muted-foreground truncate">{btn.value}</span>}
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="flex gap-2 pt-2 border-t border-border">
                        ...
                        <Button size="sm" variant="outline" onClick={onClose}
                            className="mr-auto border-border text-foreground">
                            {t.detail.close}
                        </Button>
                    </div>
                </div>
