    ...
    if (activeSubTab === "keywords") return (
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-1.5 h-9 text-sm" onClick={() => openCreate("keywords")}><Plus className="w-4 h-4" /> {tx(lang, "كلمة جديدة", "New keyword")}</Button>
        </div>
        ...
        {kwRules.length === 0 ? (
          <EmptyState icon={<MessageSquare className="w-10 h-10 text-green-300" />} title={tx(lang, "لا توجد كلمات بعد", "No keywords yet")} desc={tx(lang, "أضف كلمة مفتاحية وردّها الثابت", "Add a keyword and its fixed reply")}
            action={<Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2" onClick={() => openCreate("keywords")}><Plus className="w-4 h-4" /> {tx(lang, "أضف أول كلمة", "Add your first keyword")}</Button>} />
        ) : (
    ...
    if (activeSubTab === "welcome") return (
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          {welcomeRules.length === 0 && <Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-1.5 h-9 text-sm" onClick={() => openCreate("welcome")}><Plus className="w-4 h-4" /> {tx(lang, "إضافة ترحيب", "Add welcome")}</Button>}
        </div>
        ...
        <div className="mt-4">
          {welcomeRules.length === 0 ? (
            <EmptyState icon={<Hand className="w-10 h-10 text-green-300" />} title={tx(lang, "لا يوجد ترحيب تلقائي", "No automatic welcome yet")} desc={tx(lang, "أضف رسالة ترحيب تُرسل فور تواصل أي عميل جديد", "Add a welcome message sent as soon as any new customer contacts you")}
              action={<Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2" onClick={() => openCreate("welcome")}><Plus className="w-4 h-4" /> {tx(lang, "إضافة رسالة الترحيب", "Add welcome message")}</Button>} />
          ) : (
    ...
    if (activeSubTab === "timebased") return (
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-1.5 h-9 text-sm" onClick={() => openCreate("timebased")}><Plus className="w-4 h-4" /> {tx(lang, "جدولة جديدة", "New schedule")}</Button>
        </div>
        ...
        {timeRules.length === 0 ? (
          <EmptyState icon={<CalendarClock className="w-10 h-10 text-purple-300" />} title={tx(lang, "لا توجد جدولة بعد", "No schedules yet")} desc={tx(lang, "أضف قاعدة لإرسال رسائل في وقت وأيام محددة", "Add a rule to send messages at a specific time and days")}
            action={<Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2" onClick={() => openCreate("timebased")}><Plus className="w-4 h-4" /> {tx(lang, "إضافة جدولة", "Add schedule")}</Button>} />
        ) : (
        ...
        <DialogContent className={dialogMode === "interactive" ? "max-w-2xl max-h-[90vh] overflow-y-auto" : "max-w-md"} dir={lang === "ar" ? "rtl" : "ltr"}>
          <DialogHeader>
            <DialogTitle className="text-lg font-bold flex items-center gap-2">
              {dialogMode === "keywords" && <><Key className="w-5 h-5 text-primary" /> {editTarget ? tx(lang, "تعديل الكلمة", "Edit keyword") : tx(lang, "كلمة مفتاحية جديدة", "New keyword")}</>}
              {dialogMode === "welcome" && <><Hand className="w-5 h-5 text-primary" /> {editTarget ? tx(lang, "تعديل رسالة الترحيب", "Edit welcome message") : tx(lang, "رسالة ترحيب جديدة", "New welcome message")}</>}
