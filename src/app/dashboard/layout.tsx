...
function WhatsAppProfileView({ onBack, locale, dir }: {
  onBack: () => void;
  locale: string;
  dir: string;
}) {
...
function SettingsModal({ open, onClose, data, onSaved }: {
  ...
  // Reset sub-view when dialog closes
  useEffect(() => {
    if (!open) setShowWaProfileView(false);
  }, [open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`settings-dialog p-4 sm:p-6 ${showWaProfileView ? "max-w-2xl" : "max-w-lg"}`} dir={dir}>
        <DialogHeader>
          <DialogTitle className="text-lg font-bold flex items-center gap-2">
            <Settings className="w-5 h-5 text-[#25D366]" /> {showWaProfileView ? (locale === "ar" ? "إدارة بروفايل واتساب" : "Manage WhatsApp Profile") : s.title}
          </DialogTitle>
          <DialogDescription>
            {showWaProfileView
              ? (locale === "ar" ? "عدّل بيانات نشاطك التجاري على واتساب مباشرة من هنا" : "Edit your WhatsApp Business profile directly from here")
              : s.description}
          </DialogDescription>
        </DialogHeader>
        ...
        {showWaProfileView ? (
          ...
          <Tabs defaultValue="profile" dir={dir} className="settings-tabs min-w-0">
            <TabsList className="w-full mb-4 h-auto min-h-10 gap-1 overflow-x-auto justify-start">
              <TabsTrigger value="profile" className="flex-1 min-w-[7rem] text-xs">
                {locale === "ar" ? "وني بروفايل" : "Wani Profile"}
              </TabsTrigger>
              {isOwner && (
            ...
            {/* ── Wani Profile (Profile + Password merged, scrollable) ── */}
            <TabsContent value="profile" className="max-h-[60vh] overflow-y-auto pr-1 -mr-1">
              <div className="space-y-4">
                {/* User Card */}
                ...
                <div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800 space-y-4">
                  ...
                  {!hasPassword ? (
                    <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-xl p-3 space-y-1">
                ...
                {/* ── Account Deletion ── */}
                <div className="pt-4 mt-2 border-t border-red-100 dark:border-red-900/30 space-y-2">
                  {!showDeleteConfirm ? (
            ...
            {isOwner && (
              <TabsContent value="whatsapp" className="max-h-[60vh] overflow-y-auto pr-1 -mr-1">
                <div className="space-y-4">
                  ...
                  {data.whatsapp ? (
                    <>
                      {/* ── Manage WhatsApp Profile Button ── */}
                      <Button
                        onClick={() => setShowWaProfileView(true)}
                        variant="outline"
                        className="w-full rounded-xl border-[#25D366]/30 text-[#25D366] hover:bg-[#25D366]/5 hover:border-[#25D366]/50 font-bold gap-2 py-5 transition-all"
                      >
