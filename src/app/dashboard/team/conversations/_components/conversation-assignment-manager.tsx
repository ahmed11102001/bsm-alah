...
const inputClass = "h-10 rounded-xl border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20";
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto" dir={dir}>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 shadow-sm mb-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div className="relative md:col-span-2"><Search className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder={ar ? "ابحث بالاسم أو الرقم" : "Search conversations..."} className={`${inputClass} w-full pl-9`} /></div>
      ...
      {selectedCount > 0 && <div className="sticky top-2 z-20 bg-primary text-primary-foreground rounded-xl p-3 mb-3 flex flex-wrap items-center gap-3 shadow-lg"><span className="font-semibold text-sm">{ar ? `تم تحديد ${selectedCount}` : `Selected: ${selectedCount}`}</span><span className="text-xs opacity-80">{ar ? "من النتائج الحالية" : "from current results"}</span><select disabled={busy} value={assigneeId} onChange={e => setAssigneeId(e.target.value)} className="h-9 rounded-lg px-2 text-sm bg-background text-foreground"><option value="">{ar ? "اختر عضوًا" : "Select team member"}</option>{members.map(m => <option key={m.id} value={m.id}>{m.name || m.email}</option>)}</select><button disabled={busy || !selectedMember} onClick={() => bulkUpdate(assigneeId)} className="h-9 px-3 rounded-lg bg-primary disabled:opacity-50 text-sm font-semibold">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : ar ? "تعيين" : "Assign"}</button><button disabled={busy} onClick={() => bulkUpdate(null)} className="h-9 px-3 rounded-lg bg-white/15 hover:bg-white/25 disabled:opacity-50 text-sm">{ar ? "إلغاء التعيين" : "Unassign"}</button><button onClick={() => setSelected(new Set())} className="ml-auto p-1"><X className="w-4 h-4" /></button></div>}

      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        <div className="grid grid-cols-[36px_minmax(180px,1.3fr)_minmax(160px,2fr)_minmax(150px,1fr)] gap-3 items-center px-4 py-3 bg-muted/70 border-b border-border text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={allVisibleSelected} onChange={toggleAll} aria-label={ar ? "تحديد الكل" : "Select all visible"} /><span>{ar ? "العميل" : "Customer"}</span><span>{ar ? "آخر رسالة" : "Last message"}</span><span>{ar ? "المسؤول" : "Assigned to"}</span></div>
        {loading ? <TableRowsSkeleton rows={6} cols={2} /> : rows.length === 0 ? <div className="py-16 text-center text-sm text-muted-foreground"><Users className="w-9 h-9 mx-auto mb-2 opacity-40" />{search ? (ar ? "لا توجد نتائج للبحث" : "No conversations match your search") : assignment === "unassigned" ? (ar ? "لا توجد محادثات غير معينة" : "No unassigned conversations") : (ar ? "لا توجد محادثات" : "No conversations found")}</div> : rows.map(row => <div key={row.id} className="grid grid-cols-[36px_minmax(180px,1.3fr)_minmax(160px,2fr)_minmax(150px,1fr)] gap-3 items-center px-4 py-3 border-b border-border last:border-0 hover:bg-muted/60"><input type="checkbox" checked={selected.has(row.id)} onChange={() => toggleRow(row.id)} /><div className="min-w-0"><p className="text-sm font-semibold text-foreground truncate">{row.name || row.phone}</p><p className="text-xs text-muted-foreground truncate">{row.phone}</p></div><div className="min-w-0"><p className="text-sm text-foreground/80 truncate">{row.lastMessage?.content || (row.lastMessage ? `[${row.lastMessage.type}]` : "—")}</p><p className="text-[11px] text-muted-foreground">{row.lastMessageAt ? new Date(row.lastMessageAt).toLocaleString(ar ? "ar-EG" : "en-US") : "—"}{row.unreadCount > 0 && <span className="ml-2 text-primary">{row.unreadCount} unread</span>}</p></div><select value={row.assignedToUserId ?? ""} onChange={e => quickAssign(row.id, e.target.value || null)} className="h-9 max-w-full rounded-lg border border-border bg-background px-2 text-xs text-foreground"><option value="">{ar ? "غير معينة" : "Unassigned"}</option>{members.map(m => <option key={m.id} value={m.id}>{m.name || m.email}</option>)}</select></div>)}
      </div>
      <div className="flex items-center justify-between mt-4"><span className="text-xs text-muted-foreground">{ar ? `صفحة ${page} من ${totalPages}` : `Page ${page} of ${totalPages}`}</span><div className="flex items-center gap-2"><button disabled={page <= 1 || loading} onClick={() => { setPage(p => p - 1); setSelected(new Set()); }} className="p-2 rounded-lg border border-border disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button><button disabled={page >= totalPages || loading} onClick={() => { setPage(p => p + 1); setSelected(new Set()); }} className="p-2 rounded-lg border border-border disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button></div></div>
    </div>
  );
}
