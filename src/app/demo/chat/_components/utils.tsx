...
export function linkify(text: string) {
  ...
  for (const match of text.matchAll(URL_PATTERN)) {
    ...
    nodes.push(
      <a key={`url-${key++}`} href={href} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-blue-400 underline underline-offset-2 break-all cursor-pointer hover:text-blue-300" dir="ltr">
        {url}
      </a>
    );
