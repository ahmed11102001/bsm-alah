...
} from "@/app/dashboard/reports/_shared";
import { DEMO_REPORTS_LOGS } from "../../_lib/demo-data";

export default function DemoReportsLogsPage() {
    ...
    const fetchLogs = useCallback(async (_page = logPage) => {
        setLL(true);
        await new Promise(r => setTimeout(r, 250));
        let filtered = DEMO_REPORTS_LOGS.messages;
        if (logStatus !== "all") filtered = filtered.filter(m => m.status === logStatus);
        if (logType !== "all") filtered = filtered.filter(m => m.type === logType);
        if (logSearch.trim()) filtered = filtered.filter(m => m.contact?.phone.includes(logSearch.trim()) || m.contact?.name?.includes(logSearch.trim()));
        setLogs({ ...DEMO_REPORTS_LOGS, messages: filtered, total: filtered.length });
        setLL(false);
