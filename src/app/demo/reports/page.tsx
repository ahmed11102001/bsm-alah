...
import { DEMO_REPORTS_OVERVIEW, DEMO_REPORTS_CUSTOMERS, DEMO_REPORTS_TEAM } from "../_lib/demo-data";

export default function DemoReportsOverviewPage() {
    ...
    const fetchOverview = useCallback(async () => {
        ...
        setOverview(DEMO_REPORTS_OVERVIEW);
        setLO(false);
    }, []);

    const fetchCustomers = useCallback(async (_seg = custSegment) => {
        setLC(true);
        await new Promise(r => setTimeout(r, 250));
        setCustomers(DEMO_REPORTS_CUSTOMERS);
        setLC(false);
    }, [custSegment]);
    ...
    const fetchTeam = useCallback(async () => {
        setLT(true);
        await new Promise(r => setTimeout(r, 250));
        setTeam(DEMO_REPORTS_TEAM);
        setUnassignedCount(8);
        setLT(false);
    }, []);
                                            ...
                                            {team.map((m) => {
                                                ...
                                                return (
                                                    <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                                                        ...
                                                        <td className="py-3 px-4 text-gray-700">{formatNumber(m.assigned ?? 0, numberLocale)}</td>
                                                        <td className="py-3 px-4">
