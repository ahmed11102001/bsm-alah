...
import { StatusBadge } from "@/app/dashboard/templates/_components/StatusBadge";
import { CategoryBadge } from "@/app/dashboard/templates/_components/CategoryBadge";
import { WhatsAppPreview } from "@/app/dashboard/templates/_components/WhatsAppPreview";
import { WaniEditModal } from "@/app/dashboard/templates/_components/WaniEditModal";
import { WaniReadyCard } from "@/app/dashboard/templates/_components/WaniReadyCard";
import { TemplateDetailModal } from "@/app/dashboard/templates/_components/TemplateDetailModal";
import { Step1 } from "@/app/dashboard/templates/_components/Step1";
import { Step2 } from "@/app/dashboard/templates/_components/Step2";
import type { Template, TemplateStatus, TemplateCategory, View, Lang, FormState } from "@/app/dashboard/templates/_components/types";
import { DEMO_TEMPLATES } from "../_lib/demo-data";
...
export default function DemoTemplatesPage() {
  ...
  const handleSync = async () => {
    ...
    window.setTimeout(() => {
      setSyncing(false);
      toast.success(lang === "ar" ? `تمت المزامنة بنجاح (${templates.length} قالب)` : `Sync successful (${templates.length} templates)`);
    }, 500);
  };
  ...
  const handleSubmit = async (draft: boolean) => {
    ...
    window.setTimeout(() => {
      const next: Template = {
        ...
        status: draft ? "PAUSED" : "PENDING",
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      } as Template;
      setTemplates(prev => [next, ...prev]);
      if (!draft) setSubmitSuccess(true);
      else {
        toast.success(lang === "ar" ? "تم حفظ القالب كمسودة" : "Saved as draft");
        setView("list");
        setStep(1);
        setForm(defaultForm);
      }
  ...
  const handleSendWani = async (tpl: Template) => {
    setTemplates(prev => prev.map(tplItem => tplItem.id === tpl.id ? { ...tplItem, status: "PENDING", updatedAt: new Date().toISOString() } : tplItem));
  ...
  if (view === "list") return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6" dir={dir}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setView("create"); setStep(1); setForm(defaultForm); setSubmitSuccess(false); }}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-[#25D366] hover:bg-[#1fb956] rounded-xl shadow-sm">
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
          {["ALL", "MARKETING", "UTILITY", "AUTHENTICATION"].map(c => (
            <button key={c} onClick={() => setFilterCat(c)}
              className={`text-xs font-medium px-3 py-1.5 rounded-lg transition-all ${filterCat === c ? "bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{lang === "ar" ? "جاري تحميل القوالب..." : "Loading templates..."}</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-16 h-16 rounded-3xl bg-gray-50 dark:bg-gray-700 flex items-center justify-center mb-4">
                ...
                {filtered.map(tpl => (
                  <tr key={tpl.id} onClick={() => setDetailTpl(tpl)}
                    ...
                    <td className="px-4 py-3.5">
                      <span className="font-mono text-sm font-semibold text-gray-900 dark:text-white group-hover:text-[#25D366] transition-colors">{tpl.name}</span>
                    </td>
                    <td className="px-4 py-3.5"><CategoryBadge category={tpl.category} lang={lang} /></td>
                    <td className="px-4 py-3.5"><span className="text-xs text-gray-500 dark:text-gray-400">{tpl.language === "ar" ? "🇸🇦" : "🇬🇧"} {tpl.language}</span></td>
                    <td className="px-4 py-3.5"><span className="text-xs text-gray-400 dark:text-gray-500">{tpl.updatedAt ?? "—"}</span></td>
                    <td className="px-4 py-3.5"><StatusBadge status={tpl.status} label={t.status[tpl.status]} /></td>
                    <td className="px-4 py-3.5"><ChevronLeft className="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-gray-500 dark:group-hover:text-gray-400 transition-colors" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <WaniEditModal template={waniEditTpl} open={!!waniEditTpl} onClose={() => setWaniEditTpl(null)} onSendCustomized={handleSendWani} lang={lang} />
    </div>
  );
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6" dir={dir}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <button onClick={() => setView("list")} className="hover:text-gray-700 dark:hover:text-gray-200 transition-colors">{t.title}</button>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-white">{t.storeGroupTitle}</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t.storeGroupDesc}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
          {WANI_READY.filter(tpl => tpl.group === "store" && tpl.language === lang).map(tpl => {
            const matched = templates.find(existing => existing.name === tpl.name) ?? null;
            return (
              <WaniReadyCard key={tpl.id} template={tpl} lang={lang} onView={() => setDetailTpl(matched ?? { ...tpl, status: "NOT_SENT", createdAt: undefined, updatedAt: undefined })} onSend={handleSendWani} onCustomize={() => setWaniEditTpl(tpl)} matchedTemplate={matched} />
            );
          })}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-white">{t.followupGroupTitle}</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t.followupGroupDesc}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
          {WANI_READY.filter(tpl => tpl.group === "followup" && tpl.language === lang).map(tpl => {
            const matched = templates.find(existing => existing.name === tpl.name) ?? null;
            return (
              <WaniReadyCard key={tpl.id} template={tpl} lang={lang} onView={() => setDetailTpl(matched ?? { ...tpl, status: "NOT_SENT", createdAt: undefined, updatedAt: undefined })} onSend={handleSendWani} onCustomize={() => setWaniEditTpl(tpl)} matchedTemplate={matched} />
            );
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 mt-8">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-white">{t.campaignGroupTitle}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
          {WANI_READY.filter(tpl => tpl.group === "campaign" && tpl.category === "MARKETING" && tpl.language === lang).map(tpl => {
            const matched = templates.find(existing => existing.name === tpl.name) ?? null;
            return (
              <WaniReadyCard key={tpl.id} template={tpl} lang={lang} onView={() => setDetailTpl(matched ?? { ...tpl, status: "NOT_SENT", createdAt: undefined, updatedAt: undefined })} onSend={handleSendWani} onCustomize={() => setWaniEditTpl(tpl)} matchedTemplate={matched} />
            );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
          {WANI_READY.filter(tpl => tpl.group === "campaign" && tpl.category === "UTILITY" && tpl.language === lang).map(tpl => {
            const matched = templates.find(existing => existing.name === tpl.name) ?? null;
            return (
              <WaniReadyCard key={tpl.id} template={tpl} lang={lang} onView={() => setDetailTpl(matched ?? { ...tpl, status: "NOT_SENT", createdAt: undefined, updatedAt: undefined })} onSend={handleSendWani} onCustomize={() => setWaniEditTpl(tpl)} matchedTemplate={matched} />
            );
  ...
  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto" dir={dir}>
      <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
        <button onClick={() => { setView("list"); setStep(1); }} className="hover:text-gray-700 dark:hover:text-gray-200 transition-colors">{t.title}</button>
        <ChevronLeft className="w-3.5 h-3.5" />
      ...
      <div className="flex items-center gap-3 mb-8">
        {[{ n: 1, label: t.step1 }, { n: 2, label: t.step2 }].map((s, i) => (
          <div key={s.n} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step === s.n ? "bg-[#25D366] text-white" : step > s.n ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600" : "bg-gray-100 dark:bg-gray-700 text-gray-400"}`}>
              {step > s.n ? <CheckCircle2 className="w-3.5 h-3.5" /> : s.n}
            </div>
            <span className={`text-sm font-medium ${step === s.n ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-gray-500"}`}>{s.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
          {step === 1 ? (
            ...
            <Step2 form={form} setForm={setForm} lang={lang} onSubmit={handleSubmit} onBack={() => setStep(1)} submitting={submitting} success={submitSuccess} />
          )}
        </div>
