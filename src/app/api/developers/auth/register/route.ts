...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rl.success) return NextResponse.json({ error: "كثير من المحاولات، حاول بعد شوية" }, { status: 429 });

    const existing = await prisma.developerUser.findUnique({ where: { email: email.toLowerCase() } });
    if (existing) return NextResponse.json({ error: "الإيميل ده مسجل قبل كده" }, { status: 409 });
