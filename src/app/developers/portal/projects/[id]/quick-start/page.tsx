...
function CopyButton({ value }: { value: string }) {
  ...
  return <button onClick={async () => { await navigator.clipboard.writeText(value); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
    style={{ border: "1px solid #334155", background: "#172033", color: "#e2e8f0", borderRadius: 8, padding: "8px 12px", cursor: "pointer" }}>
...
export default function QuickStartPage() {
  ...
  return <main style={{ maxWidth: 1000, margin: "0 auto", padding: "44px 24px", color: "#f8fafc", direction: language === "ar" ? "rtl" : "ltr" }}>
    <div style={{ marginBottom: 30 }}>
      <div style={{ color: "#38bdf8", fontSize: 12, letterSpacing: 1, textTransform: "uppercase" }}>Wani Integration Generator</div>
      <h1 style={{ fontSize: 32, margin: "8px 0" }}>{t("Connect Wani to your application", "اربط Wani بتطبيقك")}</h1>
      <p style={{ color: "#94a3b8", maxWidth: 700 }}>{t("Generate deterministic, server-side integration code from the real Wani API contract.", "أنشئ كود ربط deterministic يعمل على الخادم وفق عقد Wani الفعلي.")}</p>
    </div>

    <section style={{ display: "grid", gap: 18, background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: 22 }}>
      <label>{t("What do you want to build?", "ماذا تريد أن تبني؟")}
        <select value={operation} onChange={(e) => { setOperation(e.target.value as IntegrationOperation); setCode(""); }} style={{ display: "block", width: "100%", marginTop: 8, padding: 12, background: "#111827", color: "white", borderRadius: 8 }}>
      ...
      {needsTemplate && <label>{t("Approved OTP template", "قالب OTP المعتمد")}
        <select value={selectedTemplateId} onChange={(e) => setSelectedTemplateId(e.target.value)} style={{ display: "block", width: "100%", marginTop: 8, padding: 12, background: "#111827", color: "white", borderRadius: 8 }}>
          <option value="">Select a template</option>
          {approved.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.language}</option>)}
        </select>
      </label>}

      {needsTemplate && approved.length === 0 && <div style={{ color: "#fbbf24", background: "#422006", padding: 14, borderRadius: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <label>Language<select value={lang} onChange={(e) => setLang(e.target.value as IntegrationLanguage)} style={{ display: "block", width: "100%", marginTop: 8, padding: 12, background: "#111827", color: "white", borderRadius: 8 }}>{languages.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        <label>Framework<select value={framework} onChange={(e) => setFramework(e.target.value)} style={{ display: "block", width: "100%", marginTop: 8, padding: 12, background: "#111827", color: "white", borderRadius: 8 }}>{availableFrameworks.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      </div>

      {error && <div style={{ color: "#fca5a5" }}>{error}</div>}
      <button onClick={generate} disabled={needsTemplate && !selected} style={{ width: "fit-content", padding: "12px 18px", border: 0, borderRadius: 8, background: needsTemplate && !selected ? "#334155" : "#0ea5e9", color: "white", cursor: "pointer" }}>Generate code</button>
    </section>

    {code && <section style={{ marginTop: 24, background: "#020617", border: "1px solid #1e293b", borderRadius: 16, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 16, borderBottom: "1px solid #1e293b" }}><strong>Generated integration</strong><CopyButton value={code} /></div>
      <pre style={{ margin: 0, padding: 20, overflowX: "auto", lineHeight: 1.6, color: "#dbeafe" }}><code>{code}</code></pre>
      <div style={{ padding: 20, color: "#94a3b8", lineHeight: 1.7 }}>
        <strong style={{ color: "#e2e8f0" }}>Next steps</strong>
        ...
        <div style={{ marginTop: 10, color: "#fbbf24" }}>Never expose the Wani API key or Meta credentials in browser code.</div>
      </div>
    </section>}
  </main>;
}
