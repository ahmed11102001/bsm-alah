...
import { StoreCardId, STORE_CARD_VISUALS, StepItem, ExternalLinkItem } from "./store-types";
...
export function StoreIntegrationCard({
  ...
  const v = STORE_CARD_VISUALS.find(c => c.id === id);
  ...
  return (
    <div className={cn(
      ...
      isOpen
        ...
        : "bg-card border-border hover:border-muted-foreground/60 hover:shadow-xs"
    )}>
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-3.5 min-w-0">
          <div className={cn(
            "w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform bg-muted border border-border",
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center bg-muted text-muted-foreground transition-transform duration-200", isOpen && "rotate-180")}>
      ...
      {isOpen && (
        <div className="px-4 sm:px-5 pb-5 space-y-4 border-t border-border pt-4 animate-in fade-in duration-200">
          ...
          {hasGuide && (
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-border bg-muted/80 hover:bg-muted transition text-xs font-semibold text-foreground select-none cursor-pointer"
              >
              ...
              {showGuide && (
                <div className="p-4 rounded-xl bg-muted/70 border border-border space-y-3 animate-in fade-in duration-200">
                  {steps && steps.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                      {steps.map((step, i) => (
                        <div key={i} className="bg-card rounded-xl p-3 border border-border space-y-1">
                          ...
                          <p className="text-[11px] text-muted-foreground pr-7 leading-relaxed">{step.desc}</p>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="bg-card rounded-xl p-4 sm:p-5 border border-border shadow-xs space-y-4">
            {children}
          </div>
        </div>
      )}
