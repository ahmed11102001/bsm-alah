...
// ─── كارت أتمتة عام: تفعيل/إيقاف + اختيار Template (يُستخدم لكل الأنواع) ────
export default function EmailAutomationCard({ type, icon, title, description, accent = "emerald" }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (autoRes.ok) {
          const row = (autoData.automations ?? []).find((a: any) => a.type === type);
          if (row) {
  ...
  const save = async () => {
    ...
    try {
      const r = await fetch("/api/email/automations", {
        ...
        body: JSON.stringify({ type, enabled, templateId: templateId || null }),
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(d.error || "فشل الحفظ");
      setSavedMsg(enabled ? "الأتمتة مفعّلة ✅" : "الأتمتة متوقفة");
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSaving(false);
