    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-md">
      <div className="flex items-center justify-between pb-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-white">إرشادات توثيق النطاق (SPF, DKIM, DMARC)</h3>
        </div>
        <span className="rounded-full bg-blue-500/10 border border-blue-500/20 px-2.5 py-0.5 text-[10px] font-semibold text-blue-300">
          دليل التكوين (DNS Architecture)
        </span>
      </div>

      <div className="mt-3 flex items-start gap-2.5 rounded-xl border border-white/10 bg-white/[0.02] p-3 text-xs text-white/70 leading-relaxed">
        <Info className="h-4 w-4 text-blue-400 shrink-0 mt-0.5" />
        <p>
          لتحقيق أعلى نسبة وصول لصناديق الوارد (Inbox) بدلاً من مجلد الرسائل غير المرغوب فيها (Spam)،
          تأكد من إضافة هذه السجلات في لوحة تحكم الـ DNS الخاصة بنطاقك (<span className="text-white font-mono font-semibold">{domain}</span>) لدى مزود الدومين الخاص بك (مثل Cloudflare أو GoDaddy أو Namecheap).
        </p>
      </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-bold text-white flex items-center gap-2">
                <span className="rounded bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 text-[10px] font-mono">
                  {rec.type}
                </span>
                <span>{rec.label}</span>
              </span>
              <span className="text-[11px] text-white/40 font-mono">{rec.name}</span>
            </div>

            <p className="text-[11px] text-white/50">{rec.description}</p>

            <div className="flex items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/40 px-3 py-2 font-mono text-[11px] text-white/80">
