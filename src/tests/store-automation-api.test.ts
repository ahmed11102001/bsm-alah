...
vi.mock("@/lib/whatsapp-api", () => ({
  sendWhatsAppMessage: vi.fn().mockImplementation(() =>
    Promise.resolve({ ok: true, whatsappMsgId: `msg-api-${Date.now()}-${Math.random().toString(36).slice(2, 7)}` })
  ),
}));
...
describe("Integration: Store Automation Idempotency (Real DB)", () => {
  ...
  beforeEach(async () => {
    ...
    const store = await prisma.shopifyStore.create({
      data: {
        userId: testUserId,
        shop: `test-store-api-${Date.now()}-${Math.random().toString(36).slice(2, 7)}.myshopify.com`,
