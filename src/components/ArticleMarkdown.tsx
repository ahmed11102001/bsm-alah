...
export default function ArticleMarkdown({ content, className = "" }: ArticleMarkdownProps) {
  ...
  return (
    <div
      className={`article-markdown-body text-gray-800 leading-relaxed ${className}`}
      dir="rtl"
    >
      <ReactMarkdown
        ...
        components={{
          ...
          a({ href, children, ...props }) {
            ...
            if (isInternal) {
              return (
                <Link
                  href={normalizedHref}
                  className="text-[#25D366] hover:text-[#1da851] underline font-semibold transition-colors duration-150"
                  {...props}
                >
                  {children}
                </Link>
          ...
          h1({ children }) {
            return (
              <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mt-10 mb-4 pb-2 border-b border-gray-100 leading-tight">
                {children}
              </h2>
            );
          },
          h2({ children }) {
            return (
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mt-8 mb-4 border-r-4 border-[#25D366] pr-3 leading-snug">
                {children}
              </h2>
            );
          },
          h3({ children }) {
            return (
              <h3 className="text-lg sm:text-xl font-bold text-gray-900 mt-6 mb-3 leading-snug">
                {children}
              </h3>
            );
          },
          h4({ children }) {
            return (
              <h4 className="text-base sm:text-lg font-semibold text-gray-900 mt-5 mb-2">
                {children}
              </h4>
            );
          },
          ...
          ul({ children }) {
            return (
              <ul className="list-disc list-outside pr-6 space-y-2.5 mb-6 text-gray-700">
                {children}
              </ul>
            );
          },
          ol({ children }) {
            return (
              <ol className="list-decimal list-outside pr-6 space-y-2.5 mb-6 text-gray-700">
                {children}
              </ol>
            );
          },
          ...
          blockquote({ children }) {
            return (
              <blockquote className="border-r-4 border-[#25D366] bg-green-50/50 pr-4 py-3 pl-3 rounded-l-xl my-6 text-gray-700 italic">
                {children}
              </blockquote>
            );
          },
          ...
          code({ className, children, ...props }) {
            ...
            return (
              <code
                className="bg-gray-100 text-[#128C7E] px-1.5 py-0.5 rounded font-mono text-sm"
                dir="ltr"
                {...props}
              >
                {children}
          ...
          th({ children }) {
            return (
              <th className="p-3.5 font-bold text-gray-900">
                {children}
              </th>
            );
          },
          td({ children }) {
            return <td className="p-3.5 text-gray-700">{children}</td>;
          },
            ...
            return (
              <span className="block my-6">
                <img
                  src={src}
                  alt={alt || "مقال وني"}
                  className="rounded-2xl max-w-full h-auto mx-auto shadow-md border border-gray-100"
                  loading="lazy"
                />
                {alt && (
